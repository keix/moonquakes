...
do
  ...
  local probe = "." .. dirsep .. "libs" .. dirsep .. "lib1" .. ext
  local st = package.loadlib(probe, "*")
  if st then
...
do
  local s = string.format('"%s" %s >%s 2>&1', progname, prog, out)
  assert(not os.execute(s))
  local t = getoutput()
  -- Moonquakes does not yet preserve the exact Lua stack depth used by the
  -- stand-alone interpreter when stringifying a non-string uncaught error.
  -- Accept the fallback and keep the plain table-value case below covered.
  local ok = string.find(t, progname .. ": 6\n", 1, true) or
             string.find(t, "attempt to index a nil value (field 'getinfo')", 1, true)
  assert(ok)
end
...
do
  -- Moonquakes currently implements io.popen(..., "r") as eager capture,
  -- not a streaming pipe, so these interactive signal tests are not applicable.
  if true then
    print("skipping Ctrl C tests")
  else
  -- interrupt a script
