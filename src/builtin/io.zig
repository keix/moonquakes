//! Input/output builtin functions, file-handle helpers, and buffering helpers.
//!
//! Shared helper logic stays near the top of the file.
//! Dispatcher entrypoints are being grouped below in small steps.
...
const FILE_IMPL_KEY = "_impl"; // FileObject reference for delegation
...
fn namedValueTypeName(vm: anytype, v: TValue) []const u8 {
    ...
    const mt_opt: ?*TableObject = switch (v.asObjectPtr().type) {
        .table => object.getObject(TableObject, v.asObjectPtr()).metatable,
        .userdata => object.getObject(object.UserdataObject, v.asObjectPtr()).metatable,
        .file => object.getObject(FileObject, v.asObjectPtr()).metatable,
        else => null,
    };
    if (mt_opt) |mt| {
...
pub fn initStdioHandles(io_table: *TableObject, gc: *GC) !void {
    ...
    try tableSet(gc, io_table, TValue.fromString(stdin_key), TValue.fromFile(stdin_handle));
    try tableSet(gc, io_table, TValue.fromString(stdout_key), TValue.fromFile(stdout_handle));
    try tableSet(gc, io_table, TValue.fromString(stderr_key), TValue.fromFile(stderr_handle));
    try tableSet(gc, io_table, TValue.fromString(default_input_key), TValue.fromFile(stdin_handle));
    try tableSet(gc, io_table, TValue.fromString(default_output_key), TValue.fromFile(stdout_handle));
}
...
fn createStdioHandleInit(gc: *GC, stdio_type: []const u8) !*TableObject {
    ...
    try tableSet(gc, file_table, TValue.fromString(stdio_key), TValue.fromString(stdio_str));

    const output_key = try gc.allocString(FILE_OUTPUT_KEY);
    const empty_str = try gc.allocString("");
    try tableSet(gc, file_table, TValue.fromString(output_key), TValue.fromString(empty_str));

    const closed_key = try gc.allocString(FILE_CLOSED_KEY);
    try tableSet(gc, file_table, TValue.fromString(closed_key), TValue.fromBool(false));
    ...
    try tableSet(gc, file_table, TValue.fromString(mode_key), TValue.fromString(mode_str));
    ...
    try tableSet(gc, file_table, TValue.fromString(bufmode_key), TValue.fromString(bufmode_str));

    const pos_key = try gc.allocString(FILE_POS_KEY);
    try tableSet(gc, file_table, TValue.fromString(pos_key), TValue.fromInt(0));

    setTableMetatable(gc, file_table, try createFileMetatableInit(gc));
    return file_table;
}
...
fn createStdioFileObjectInit(gc: *GC, stdio_type: []const u8) !*FileObject {
    ...
    setFileStringRef(gc, file_obj, &file_obj.mode, try gc.allocString(if (kind == .stdin) "r" else "w"));
    setFileMetatable(gc, file_obj, try createFileMetatableInit(gc));
    return file_obj;
}
...
fn createStdioHandle(vm: anytype, temp_slot: u32, stdio_type: []const u8) !*TableObject {
    ...
    setTableMetatable(vm.gc(), file_table, mt);
...
fn parseLuaNumberToken(str: []const u8) ?TValue {
    ...
    if (parseHexIntegerWrapLocal(str)) |i| return TValue.fromInt(i);
    if (parseHexFloatLocal(str)) |n| return TValue.fromFloat(n);
    if (std.fmt.parseInt(i64, str, 10)) |i| return TValue.fromInt(i) else |_| {}
    if (std.fmt.parseFloat(f64, str)) |n| return TValue.fromFloat(n) else |_| {}
    return null;
}
...
fn readLuaNumberFromContent(content: []const u8, start_pos: usize) ReadNumberResult {
    ...
    if (parseLuaNumberToken(token)) |num| {
        ...
        if (num.isNumber() and !std.math.isFinite(num.asFloat())) {
            return .{ .value = null, .new_pos = i };
        }
...
fn tryHandleMultiRead(
    ...
    const pos_key = try vm.gc().allocString(FILE_POS_KEY);
    const pos_val = file_table.get(TValue.fromString(pos_key)) orelse TValue.fromInt(0);
    ...
    while (out_idx < fmt_count) : (out_idx += 1) {
        ...
        const fmt = canonicalReadFormat(fmt_obj.asSlice()) orelse return vm.raiseString("invalid format");
    ...
    try tableSet(vm.gc(), file_table, TValue.fromString(pos_key), TValue.fromInt(@intCast(cur_pos)));
    return true;
}

fn canonicalReadFormat(fmt: []const u8) ?[]const u8 {
    if (fmt.len == 0) return null;
    if (fmt[0] == '*') {
        if (fmt.len == 2 and (fmt[1] == 'a' or fmt[1] == 'l' or fmt[1] == 'L' or fmt[1] == 'n')) {
            return fmt[1..2];
        }
        if (std.mem.eql(u8, fmt, "*all")) return "a";
        if (std.mem.eql(u8, fmt, "*line")) return "l";
        if (std.mem.eql(u8, fmt, "*number")) return "n";
        return null;
    }
    if (fmt.len == 1 and (fmt[0] == 'a' or fmt[0] == 'l' or fmt[0] == 'L' or fmt[0] == 'n')) {
        return fmt;
    }
    if (std.mem.eql(u8, fmt, "all")) return "a";
    if (std.mem.eql(u8, fmt, "line")) return "l";
...
fn flushToDestination(vm: anytype, file_table: *TableObject) !void {
    ...
    if (file_table.get(TValue.fromString(stdio_key))) |stdio_val| {
        if (stdio_val.asString()) |stdio_str| {
            ...
            try tableSet(vm.gc(), file_table, TValue.fromString(output_key), TValue.fromString(empty));
            // Reset position to 0
            const pos_key = try vm.gc().allocString(FILE_POS_KEY);
            try tableSet(vm.gc(), file_table, TValue.fromString(pos_key), TValue.fromInt(0));
            return;
...
fn refreshReadableFileTable(vm: anytype, file_table: *TableObject) !void {
    ...
    const filename_key = try vm.gc().allocString(FILE_FILENAME_KEY);
    const filename_val = file_table.get(TValue.fromString(filename_key)) orelse return;
    const filename_obj = filename_val.asString() orelse return;
    const file = std.fs.cwd().openFile(filename_obj.asSlice(), .{}) catch return;
    defer file.close();

    const content = file.readToEndAlloc(vm.gc().allocator, 10 * 1024 * 1024) catch return;
...
fn createLinesIteratorWrapper(vm: anytype, temp_slot: u32, state_table: *TableObject) !*TableObject {
    ...
    try tableSet(vm.gc(), wrapper, TValue.fromString(state_key), TValue.fromTable(state_table));
    const done_key = try vm.gc().allocString("_lines_done");
    try tableSet(vm.gc(), wrapper, TValue.fromString(done_key), TValue.fromBool(false));
    ...
    if (vm.globals().get(TValue.fromString(io_key))) |io_val| {
        if (io_val.asTable()) |io_table| {
            ...
            if (io_table.get(TValue.fromString(default_output_key))) |out_val| {
                // Handle FileObject-based default output
                if (out_val.asFile()) |file_obj| {
                    if (file_obj.closed) {
                ...
                // Handle table-based default output
...
pub fn nativeIoClose(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    // Handle FileObject-based files first
    if (nargs >= 1) {
        ...
        if (arg.asTable()) |file_table| {
            ...
            if (file_table.get(TValue.fromString(impl_key))) |impl_val| {
                if (impl_val.asFile()) |file_obj| {
                    if (file_obj.closed) {
                        return vm.raiseString("closed file");
                    }
                    file_obj.close() catch {
                    ...
                    const closed_key = try vm.gc().allocString(FILE_CLOSED_KEY);
                    try tableSet(vm.gc(), file_table, TValue.fromString(closed_key), TValue.fromBool(true));
                    if (nresults > 0) vm.stack[vm.base + func_reg] = TValue.fromBool(true);
                    return;
                }
            }
        ...
        // Check if default output is a FileObject or table with _impl
        const io_key = try vm.gc().allocString("io");
        if (vm.globals().get(TValue.fromString(io_key))) |io_val| {
            if (io_val.asTable()) |io_table| {
                const default_output_key = try vm.gc().allocString(IO_DEFAULT_OUTPUT_KEY);
                if (io_table.get(TValue.fromString(default_output_key))) |output_val| {
                    // Direct FileObject
                    if (output_val.asFile()) |file_obj| {
                        // Standard files cannot be closed
                        if (file_obj.isStdio()) {
                            if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
                            return;
                        }
                    ...
                    // Table with _impl delegation
                    if (output_val.asTable()) |out_table| {
                        const impl_key = try vm.gc().allocString(FILE_IMPL_KEY);
                        if (out_table.get(TValue.fromString(impl_key))) |impl_val| {
                            if (impl_val.asFile()) |file_obj| {
                                if (file_obj.closed) {
                                    if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
                                    if (nresults > 1) {
                                ...
                                // Sync table's closed flag
                                const closed_key = try vm.gc().allocString(FILE_CLOSED_KEY);
                                try tableSet(vm.gc(), out_table, TValue.fromString(closed_key), TValue.fromBool(true));
                                if (nresults > 0) vm.stack[vm.base + func_reg] = TValue.fromBool(true);
                                return;
                            }
                        }
...
pub fn nativeIoFlush(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    // Check if default output is a FileObject that needs flushing
    const io_key = try vm.gc().allocString("io");
    if (vm.globals().get(TValue.fromString(io_key))) |io_val| {
        if (io_val.asTable()) |io_table| {
            const default_output_key = try vm.gc().allocString(IO_DEFAULT_OUTPUT_KEY);
            if (io_table.get(TValue.fromString(default_output_key))) |output_val| {
                ...
                // Handle table-based output
                if (output_val.asTable()) |out_table| {
                    try flushBufferedFileTable(vm, out_table);
                    if (nresults > 0) vm.stack[vm.base + func_reg] = TValue.fromBool(true);
                    return;
                }
            }
    ...
    // Default: stdout is unbuffered, return true for compatibility
    if (nresults > 0) {
        vm.stack[vm.base + func_reg] = TValue.fromBool(true);
    }
}
...
pub fn nativeIoInput(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (nargs == 0) {
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), TValue.fromTable(stdin_handle));

        // Also set io.stdin
        const stdin_key = try vm.gc().allocString("stdin");
        try tableSet(vm.gc(), io_table, TValue.fromString(stdin_key), TValue.fromTable(stdin_handle));
    ...
    if (arg.asString()) |filename_str| {
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), TValue.fromTable(file_table));
    ...
    if (arg.asFile()) |_| {
        try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), arg);
...
pub fn nativeIoLines(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (use_default_input) {
        ...
        if (io_table.get(TValue.fromString(default_input_key))) |in_val| {
            if (in_val.asTable()) |t| {
                ...
                try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), TValue.fromTable(file_table));
                const stdin_key = try vm.gc().allocString("stdin");
                try tableSet(vm.gc(), io_table, TValue.fromString(stdin_key), TValue.fromTable(file_table));
            }
        } else {
            file_table = try createStdioHandle(vm, func_reg + 3, "stdin");
            try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), TValue.fromTable(file_table));
        ...
        try tableSet(vm.gc(), state_table, TValue.fromString(content_key), TValue.fromString(content_obj));

        const file_pos_key = try vm.gc().allocString(FILE_POS_KEY);
        const file_pos_val = file_table.get(TValue.fromString(file_pos_key)) orelse TValue.fromInt(0);
        try tableSet(vm.gc(), state_table, TValue.fromString(pos_key), file_pos_val);

        const file_ref_key = try vm.gc().allocString("_file_ref");
        try tableSet(vm.gc(), state_table, TValue.fromString(file_ref_key), TValue.fromTable(file_table));
    } else {
        // io.lines(filename, ...)
        ...
        try tableSet(vm.gc(), file_table, TValue.fromString(pos2_key), TValue.fromInt(0));
    ...
    if (fmt_count > 0) {
        ...
        while (i < fmt_count) : (i += 1) {
            ...
            try tableSet(vm.gc(), fmts_table, TValue.fromInt(@as(i64, @intCast(i)) + 1), fmt_val);
        }
    }
...
pub fn nativeIoLinesIterator(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (fmt_count > 0) {
        ...
        try tableSet(vm.gc(), state_table, TValue.fromString(pos_key), TValue.fromInt(@intCast(cur_pos)));
        if (vm.stack[vm.base + func_reg].isNil()) {
            if (wrapper_opt) |wrapper| {
                const done_key = try vm.gc().allocString("_lines_done");
                try tableSet(vm.gc(), wrapper, TValue.fromString(done_key), TValue.fromBool(true));
            }
        }
    ...
    try tableSet(vm.gc(), state_table, TValue.fromString(pos_key), TValue.fromInt(new_pos));
    vm.top = vm.base + func_reg + 1;
}
...
pub fn nativeIoOpen(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (primary_mode == 'r') {
        ...
        const file = std.fs.cwd().openFile(filename, .{}) catch {
            ...
            if (nresults > 2) vm.stack[vm.base + func_reg + 2] = TValue.fromInt(1);
            return;
        };
        defer file.close();
        ...
        try tableSet(vm.gc(), file_table, TValue.fromString(output_key), TValue.fromString(output_str));
        ...
        try tableSet(vm.gc(), file_table, TValue.fromString(exitcode_key), TValue.fromInt(0));
    ...
    if (primary_mode == 'w') {
        // Open/create file with truncation
        const os_file = std.fs.cwd().createFile(filename, .{}) catch {
            if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
            if (nresults > 1) {
                const err_str = try vm.gc().allocString("cannot open file");
                vm.stack[vm.base + func_reg + 1] = TValue.fromString(err_str);
        ...
        // GC root: allocTable/allocString below may trigger collection.
        ...
        // Store FileObject reference for delegation
        const impl_key = try vm.gc().allocString(FILE_IMPL_KEY);
    ...
    if (primary_mode == 'a') {
        ...
        const os_file = std.fs.cwd().createFile(filename, .{ .truncate = false }) catch {
            if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
            if (nresults > 1) {
                const err_str = try vm.gc().allocString("cannot open file");
                vm.stack[vm.base + func_reg + 1] = TValue.fromString(err_str);
...
pub fn nativeIoOutput(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (nargs == 0) {
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_output_key), TValue.fromTable(stdout_handle));

        // Also set io.stdout
        const stdout_key = try vm.gc().allocString("stdout");
        try tableSet(vm.gc(), io_table, TValue.fromString(stdout_key), TValue.fromTable(stdout_handle));
    ...
    if (arg.asString()) |filename_str| {
        ...
        if (io_table.get(TValue.fromString(default_output_key))) |old_val| {
            ...
            } else if (old_val.asFile()) |old_file| {
                old_file.flush() catch {};
            }
        }

        // Open/create file with truncation (same behavior as io.open(..., "w")).
        const os_file = std.fs.cwd().createFile(filename, .{}) catch {
            return vm.raiseString("cannot open file");
        };
        const file_obj = try vm.gc().allocFile(os_file);
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_output_key), TValue.fromTable(file_table));
    ...
    if (arg.asFile()) |_| {
        ...
        if (io_table.get(TValue.fromString(default_output_key))) |old_val| {
            if (old_val.asFile()) |old_file| {
                if (old_val.asObjectPtr() != arg.asObjectPtr()) {
                    old_file.flush() catch {};
                }
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_output_key), arg);
        if (nresults > 0) vm.stack[vm.base + func_reg] = arg;
        return;
    }
...
pub fn nativeIoPopen(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    // Store exit code
    const exitcode_key = try vm.gc().allocString(FILE_EXITCODE_KEY);
    try tableSet(vm.gc(), file_table, TValue.fromString(exitcode_key), TValue.fromInt(if (result) |res| res.exit_code else 0));
    ...
    try tableSet(vm.gc(), file_table, TValue.fromString(mode_key), TValue.fromString(mode_str_obj));

    const popen_cmd_key = try vm.gc().allocString(FILE_POPEN_CMD_KEY);
    const cmd_copy = try vm.gc().allocString(cmd);
    try tableSet(vm.gc(), file_table, TValue.fromString(popen_cmd_key), TValue.fromString(cmd_copy));
    const popen_mode_key = try vm.gc().allocString(FILE_POPEN_MODE_KEY);
    try tableSet(vm.gc(), file_table, TValue.fromString(popen_mode_key), TValue.fromString(mode_str_obj));
...
pub fn nativeIoRead(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (io_table.get(TValue.fromString(default_input_key))) |input_val| {
        ...
        try tableSet(vm.gc(), io_table, TValue.fromString(default_input_key), input_handle);
        const stdin_key = try vm.gc().allocString("stdin");
        try tableSet(vm.gc(), io_table, TValue.fromString(stdin_key), input_handle);
    }
...
pub fn nativeIoTmpfile(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    const tmpfile_key = try vm.gc().allocString(FILE_TMPFILE_KEY);
    try tableSet(vm.gc(), file_table, TValue.fromString(tmpfile_key), TValue.fromBool(true));
...
pub fn nativeFileClose(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    // Check for FileObject delegation via _impl
    const impl_key = try vm.gc().allocString(FILE_IMPL_KEY);
    if (file_table.get(TValue.fromString(impl_key))) |impl_val| {
        if (impl_val.asFile()) |file_obj| {
            // Check if already closed
            ...
            // Close the FileObject (flushes and closes OS handle)
            file_obj.close() catch {
                if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
                if (nresults > 1) {
                    const err_str = try vm.gc().allocString("close error");
    ...
    if (file_table.get(TValue.fromString(popen_cmd_key))) |cmd_val| {
        if (cmd_val.asString()) |cmd_str| {
            ...
            if (std.mem.eql(u8, popen_mode, "w")) {
                ...
                const exitcode_key = try vm.gc().allocString(FILE_EXITCODE_KEY);
                try tableSet(vm.gc(), file_table, TValue.fromString(exitcode_key), TValue.fromInt(code));
            }
        }
    }
    ...
    const ok: TValue = if (exit_code == 0) TValue.fromBool(true) else .nil;
    ...
    if (nresults > 2) {
        vm.stack[vm.base + func_reg + 2] = TValue.fromInt(code_out);
    }
}
...
pub fn nativeFileFlush(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (file_table.get(TValue.fromString(impl_key))) |impl_val| {
        if (impl_val.asFile()) |file_obj| {
            ...
            if (nresults > 0) vm.stack[vm.base + func_reg] = self_arg;
            return;
        }
    }
...
pub fn nativeFileLines(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (nresults > 3) {
        vm.stack[vm.base + func_reg + 3] = .nil;
    }
}
...
pub fn nativeFileRead(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (std.mem.eql(u8, format, "a")) {
        ...
        try tableSet(vm.gc(), file_table, TValue.fromString(pos_key), TValue.fromInt(@intCast(content.len)));
        ...
        try tableSet(vm.gc(), file_table, TValue.fromString(pos_key), TValue.fromInt(new_pos));
    } else if (std.mem.eql(u8, format, "L")) {
...
pub fn nativeFileSeek(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    if (self_arg.asFile()) |file_obj| {
        ...
        if (file_obj.closed) {
            ...
            if (nresults > 2) vm.stack[vm.base + func_reg + 2] = TValue.fromInt(9); // EBADF
            return;
        }
        ...
        if (file_obj.kind == .stdin or file_obj.kind == .stdout or file_obj.kind == .stderr) {
            ...
            if (nresults > 2) vm.stack[vm.base + func_reg + 2] = TValue.fromInt(29); // ESPIPE
            return;
        }

        // Keep seek position compatible with buffered writes.
        if (file_obj.buffer.items.len > 0) {
            file_obj.flush() catch {
                if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
                if (nresults > 1) {
        ...
        if (file_obj.handle) |handle| {
            ...
            const abs_pos: u64 = switch (whence_mode) {
                .set => blk: {
                    if (offset < 0) {
                        ...
                        if (nresults > 2) vm.stack[vm.base + func_reg + 2] = TValue.fromInt(22); // EINVAL
                        return;
                    }
            ...
            if (nresults > 0) vm.stack[vm.base + func_reg] = TValue.fromInt(@intCast(new_pos));
            return;
        } else {
            // No handle - use buffer position
            // For files that are buffered only, track position in buffer
            // For now, return 0 (start of buffer)
            if (nresults > 0) vm.stack[vm.base + func_reg] = TValue.fromInt(0);
            return;
        }
    }
    ...
    if (file_table.get(TValue.fromString(impl_key))) |impl_val| {
        if (impl_val.asFile()) |file_obj| {
            vm.stack[vm.base + func_reg + 1] = TValue.fromFile(file_obj);
            return nativeFileSeek(vm, func_reg, nargs, nresults);
        }
    }
    ...
    // Return new position
    if (nresults > 0) {
        vm.stack[vm.base + func_reg] = TValue.fromInt(new_pos);
    }
}
...
pub fn nativeFileSetvbuf(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    const bufmode: object.BufMode = if (std.mem.eql(u8, mode, "no"))
        .no
    else if (std.mem.eql(u8, mode, "full"))
        .full
    else if (std.mem.eql(u8, mode, "line"))
        .line
    else {
        if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
        if (nresults > 1) {
    ...
    // Handle table-based file handle
    const file_table = self_arg.asTable() orelse {
        if (nresults > 0) vm.stack[vm.base + func_reg] = .nil;
        return;
    };
    ...
    const bufmode_key = try vm.gc().allocString(FILE_BUFMODE_KEY);
    try tableSet(vm.gc(), file_table, TValue.fromString(bufmode_key), TValue.fromString(mode_str));
    if (bufmode == .no) {
...
pub fn nativeFileWrite(vm: anytype, func_reg: u32, nargs: u32, nresults: u32) !void {
    ...
    try tableSet(vm.gc(), file_table, TValue.fromString(output_key), TValue.fromString(new_str));
    try tableSet(vm.gc(), file_table, TValue.fromString(pos_key), TValue.fromInt(@intCast(end_pos)));
