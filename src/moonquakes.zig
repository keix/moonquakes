...
/// Command-line interface for Moonquakes.
/// Separated from main.zig for testability and extensibility.
pub const CLI = struct {
    allocator: std.mem.Allocator,
    mq: *Moonquakes,

    pub fn init(allocator: std.mem.Allocator, mq: *Moonquakes) CLI {
        return .{
            .allocator = allocator,
            .mq = mq,
        };
    ...
    pub fn run(self: *CLI, args: []const [:0]const u8) !void {
        ...
        const file_path = arg;
        var result = self.mq.loadFile(file_path) catch |err| switch (err) {
            error.FileNotFound => {
                var stderr_writer = std.fs.File.stderr().writer(&.{});
                const stderr = &stderr_writer.interface;
