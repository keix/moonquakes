...
test "ADD instruction with side effect verification" {
    ...
    try utils.testSingleInstruction(
        Instruction.initABC(.ADD, 0, 1, 2),
        &[_]TValue{}, // no constants needed
        &initial_regs,
        &expected_regs,
        0, // expected base
        4  // expected top
...
test "EQ comparison with skip verification" {
    ...
    try utils.ComparisonTest.expectSkip(
        &vm,
        Instruction.initABC(.EQ, 0, 0, 1), // if (R0 == R1) == (A==0) then skip
        TValue{ .integer = 42 },
        TValue{ .integer = 42 },
        &[_]TValue{}
    );
    ...
    try utils.ComparisonTest.expectNoSkip(
        &vm,
        Instruction.initABC(.EQ, 1, 0, 1), // if (R0 == R1) == (A==0) then skip
        TValue{ .integer = 42 },
        TValue{ .integer = 42 },
        &[_]TValue{}
    );
