...
fn expectInf(result: anyerror!ReturnValue) !void {
    const ret = result catch return error.TestUnexpectedError;
    const val = switch (ret) {
        .single => |v| v,
        .multiple => |vs| if (vs.len > 0) vs[0] else return error.TestExpectedValue,
...
test "DIV: division by zero (integer) returns inf" {
    ...
    const code = [_]Instruction{
        ...
        Instruction.initABC(.DIV, 2, 0, 1), // R2 = R0 / R1 (10 / 0 = inf)
...
test "DIV: division by zero (float) returns inf" {
    var ctx: test_utils.TestContext = undefined;
    try ctx.init();
    defer ctx.deinit();
    ...
    const code = [_]Instruction{
        ...
        Instruction.initABC(.DIV, 2, 0, 1), // R2 = R0 / R1 (10.5 / 0.0 = inf)
        Instruction.initABC(.RETURN, 2, 2, 0), // return R2
    };
...
test "DIVK: division by zero constant returns inf" {
    var ctx: test_utils.TestContext = undefined;
    try ctx.init();
    defer ctx.deinit();
    ...
    const code = [_]Instruction{
        ...
        Instruction.initABC(.DIVK, 1, 0, 1), // R1 = R0 / K[1] (25 / 0.0 = inf)
